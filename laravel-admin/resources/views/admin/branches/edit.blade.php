            ...
            <div class="card">
                ...
                <div style="display:flex;gap:14px;flex-wrap:wrap;align-items:end;margin-bottom:16px;">
                    <div class="form-group" style="flex:1;min-width:130px;">
                        ...
                        <input type="time" name="schedule_open" value="{{ old('schedule_open', $branch->schedule['open'] ?? '07:00') }}">
                    </div>
                    <div class="form-group" style="flex:1;min-width:130px;">
                        <label>Cierre</label>
                        <input type="time" name="schedule_close" value="{{ old('schedule_close', $branch->schedule['close'] ?? '14:00') }}">
                    </div>
                </div>
                @php
                    $branchDays = $branch->schedule['days'] ?? ['monday','tuesday','wednesday','thursday','friday','saturday','sunday'];
                    $dayLabels = [
                        'monday' => 'Lunes',
                        'tuesday' => 'Martes',
                        'wednesday' => 'Miércoles',
                    ...
                    @foreach($dayLabels as $dayKey => $dayLabel)
                        <label style="display:flex;align-items:center;gap:5px;padding:7px 14px;border:2px solid #e0e0e0;border-radius:8px;cursor:pointer;font-size:0.8rem;font-weight:600;color:#555;transition:all 0.2s;background:#fafafa;user-select:none;">
                            <input type="checkbox" name="days[]" value="{{ $dayKey }}"
                                   {{ in_array($dayKey, old('days', $branchDays)) ? 'checked' : '' }}
